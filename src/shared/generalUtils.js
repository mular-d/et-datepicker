...
/*
  These utility functions don't depend on locale of the date picker(Ethiopic or Gregorian)
*/
...
const getDateAccordingToMonth = (date, direction, monthCount = 12) => {
  const toSum = direction === "NEXT" ? 1 : -1;
  let newMonthIndex = date.month + toSum;
  let newYear = date.year;
  if (newMonthIndex < 1) {
    newMonthIndex = monthCount;
    newYear -= 1;
  }
  if (newMonthIndex > monthCount) {
    newMonthIndex = 1;
    newYear += 1;
  }
